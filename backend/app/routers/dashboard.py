...
def get_stats(db: Session = Depends(get_mysql), _user=Depends(get_current_user)):
    ...
    # Ricavi per mese (ultimi 12 mesi, escluse NC/ND)
    fatturato_mensile = db.execute(text("""
        SELECT
          DATE_FORMAT(data_docum, '%%Y-%%m') as mese,
          COUNT(*) as num_fatture,
          COALESCE(SUM(importo), 0) as totale
        FROM gefadcts
        WHERE (cancellato = 0 OR cancellato IS NULL)
          AND tipo_fattu NOT IN ('NC', 'ND')
          AND data_docum >= DATE_SUB(NOW(), INTERVAL 12 MONTH)
        GROUP BY DATE_FORMAT(data_docum, '%%Y-%%m')
        ORDER BY mese ASC
    """)).fetchall()
    ...
    return {
        ...
        "fatturato_mensile": [dict(r._mapping) for r in fatturato_mensile],
